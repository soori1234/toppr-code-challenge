...
db.open(function(err, db) {
    if(!err) {
        ...
        db.collection('got', {strict:true}, function(err, collection) {
            if (err) {
                console.log("The 'got' collection doesn't exist. Creating it with sample data...");
                populateDB();
            }
        });
    }
...
app.get('/count.html', function (req, res) {
   res.sendFile( __dirname + "/" + "list.html" );
})

app.get("/got/count", function(req, res) {
	
			 db.collection('got').count({}, function(error, numOfDocs){
            if(error) return callback(err);

            res.send(200,numOfDocs);
			});
	
       
});

app.get("/got/list", function(req, res) {
		db.collection('got', function(err, collection) {
...
app.get("got/battles/:id", function(req, res) {
});
...
//db.createCollection('battles');
    db.collection('got', function(err, collection) {
        collection.insert(got, {safe:true}, function(err, result) {});
